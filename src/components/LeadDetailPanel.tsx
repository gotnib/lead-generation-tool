...
export default function LeadDetailPanel({ lead, onClose, onUpdate, onDelete }: Props) {
  ...
  useEffect(() => {
    ...
    const refWidth = previewMode === 'mobile' ? 393 : 1280;
        ...
        <div className="fixed inset-y-0 left-0 z-40 flex flex-col" style={{ right: '32rem' }}>
          ...
          <div className="relative z-10 flex flex-1 items-center justify-center overflow-hidden p-6">
            {previewMode === 'desktop' ? (
              ...
              /* ── Phone — iframe rendered at 393×852 (iPhone 15 Pro logical px) then scaled ── */
              <div
                className="relative overflow-hidden rounded-[2.8rem] border border-white/20 shadow-[0_24px_64px_rgba(0,0,0,0.7)]"
                style={{ width: 'min(393px, calc(100% - 3rem))', aspectRatio: '393/852' }}
              >
